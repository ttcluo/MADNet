...
class IconVSR(nn.Module):
    ...
    def __init__(self,
        ...
        # propagation
        self.backward_conv = ConvResidualBlocks(3, num_feat, 1)  # 新加的

        self.backward_fusion = nn.Conv2d(2 * num_feat, num_feat, 3, 1, 1, bias=True)
        # self.backward_trunk = ConvResidualBlocks(num_feat + 3, num_feat, num_block)  # 原始的
        self.backward_trunk = ConvResidualBlocks(num_feat*2, num_feat, num_block)

        self.forward_conv = ConvResidualBlocks(3, num_feat, 1)  # 新加的

        self.forward_fusion = nn.Conv2d(2 * num_feat, num_feat, 3, 1, 1, bias=True)
        # self.forward_trunk = ConvResidualBlocks(2 * num_feat + 3, num_feat, num_block)  # 原始的
    ...
    def get_keyframe_feature(self, x, keyframe_idx):
        ...
        feats_keyframe = {}
        for i in keyframe_idx:  # 用EDVR-M配准这四个keyframe 【0 5 10 14】
    ...
    def forward(self, x):
        ...
        flows_forward, flows_backward = self.get_flow(x)
        feats_keyframe = self.get_keyframe_feature(x, keyframe_idx)  # 相当于先用EDVR-M配准了一遍keyframe，再传播
        # keyframe 长度为4，每个元素为 【b 64 h w】

        # 改进思路1，xi与featpro串联之前，先用Kernel bias block 处理xi
        # 改进思路2，xi与featprop串联后过个resblock，就是trunk， 之后再用kernel bias block处理
        ...
        for i in range(n - 1, -1, -1):  # 14， 13， ···，
            ...
            # feat_prop = torch.cat([x_i, feat_prop], dim=1)  # 原始
            feat_prop = torch.cat([self.kbb(self.backward_conv(x_i)), feat_prop], dim=1)
            feat_prop = self.backward_trunk(feat_prop)
            out_l.insert(0, feat_prop)
        ...
        for i in range(0, n):
            ...
            # feat_prop = torch.cat([x_i, out_l[i], feat_prop], dim=1)  # 原始代码
            feat_prop = torch.cat([self.kbb(self.forward_conv(x_i)), out_l[i], feat_prop], dim=1)
            feat_prop = self.forward_trunk(feat_prop)
...
def KBA(x, att, selfk, selfg, selfb, selfw):
...
class KBBlock_s(nn.Module):
    def __init__(self, c=64, DW_Expand=2, FFN_Expand=2, nset=64, k=3, gc=4, lightweight=False):
        ...
        # self.ca = ChannelAttention(num_feat=c)
        self.multi_spectral_attention = MultiSpectralAttentionLayer(channel=c, dct_w=56, dct_h=56)
    ...
    def forward(self, inp):
        ...
        x = self.dwconv_k5(x)
        # x将输入三个分支
        # ca = self.ca(x)  # 分支1， 通道注意力，可以换成简化的通道注意力sca
        ca = self.multi_spectral_attention(x)  # 分支1，换改进的通道注意力msa

        x1 = self.conv11(x)  # 分支2， 卷积增加bias

        # 分支3， Kernel bias attention
        att = self.conv2(x) * self.attgamma + self.conv211(x)
        uf = self.conv21(self.conv1(x))

        kba = KBA(uf, att, self.k, self.g, self.b, self.w) * self.ga1 + uf
...
if __name__ == '__main__':
    ...
    # input = torch.rand(1, 64, 160, 160).cuda()  # B C H W
    model = IconVSR().cuda()
    # model = KBBlock_s().cuda()
    # output = model(input)
    print_network(model)
    # print_network(ChannelAttention(num_feat=64, squeeze_factor=16))

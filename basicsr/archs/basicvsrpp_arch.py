...
class BasicVSRPlusPlus(nn.Module):
    ...
    def propagate(self, feats, flows, module_name):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            # --------------------- visualization ------------
            temp = feat_current  # 1 64 128 128
            print(666, temp.size())
            # temp = temp.squeeze(0)
            temp = temp.reshape(1, 64, 160, 160)
            v1 = temp[0, :, :, :].cpu()  # [30 128 128]
            num = 0
            for i in range(64):
                plt.figure(figsize=(6, 5))
                map = v1[i, :, :]  # [128 128]
                print(2323, map.shape)
                map = nn.functional.interpolate(map.unsqueeze(0).unsqueeze(0), (256, 256), mode='bicubic',
                                                align_corners=True)
                # sns.heatmap(map)
                # map = torch.nn.functional.normalize(map, dim=-1)
                plt.imshow(map.squeeze(0).squeeze(0), cmap='viridis')
                # plt.colorbar()
                #
                # plt.show()
                ...
                num = num + 1
                plt.axis('off')
                name = str(num) + '_' + str(i)
                plt.savefig('D:/VSR-work3/feature-vis/001/{}.png'.format(name), dpi=600, bbox_inches='tight',
                            pad_inches=0.0)
            # sns.heatmap(map)
            plt.show()
            if self.cpu_cache:
            ...
            if i > 0 and self.is_with_alignment:
                ...
                # # print(132, feat_prop.size())
                # # --------------------- visualization ------------
                # temp = feat_prop  # 1 64 128 128
                # print(666, temp.size())
                # # temp = temp.squeeze(0)
                # temp = temp.reshape(1, 64, 160, 160)
                # v1 = temp[0, :, :, :].cpu()  # [30 128 128]
                ...
                #     plt.axis('off')
                #     name = str(num) + '_' + str(i)
                #     plt.savefig('D:/VSR-work3/feature-vis/001-align/{}.png'.format(name), dpi=600, bbox_inches='tight',
                #                 pad_inches=0.0)
                # # sns.heatmap(map)
                # plt.show()
            ...
            #     plt.axis('off')
            #     name = str(num) + '_' + str(i)
            #     plt.savefig('D:/VSR-work3/feature-vis/002-residual/{}.png'.format(name), dpi=600, bbox_inches='tight',
            #                 pad_inches=0.0)
            # # sns.heatmap(map)
            # plt.show()
